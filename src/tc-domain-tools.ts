...
const domains: DomainDef[] = [
  ...
  },
  {
    ...
    actions: {
      ...
      node_create: { method: "POST", path: "/forests/{id}/trees/{subId}/nodes", apiType: "org", hint: "id=forestId, subId=treeId, body: {name, parentId?, links?: string[], add?: string[]}. links/add = FRN strings e.g. frn:entity:GUID (%24 for $)" },
      node_update: { method: "PATCH", path: "/forests/{id}/trees/{subId}/nodes/{extraId}", apiType: "org", hint: "id=forestId, subId=treeId, extraId=nodeId. body: {add: [frn:entity:...]} to link BIM objects, {links: [...]} to replace all links, {remove: [...]} to unlink. count is read-only — setting description alone does NOT link objects" },
      changeset: { method: "POST", path: "/forests/{id}/trees/{subId}/changeset", apiType: "org", hint: "id=forestId, subId=treeId, body: {snapshot?: false, nodes: [{id, add: [frn:entity:...]}, ...]}. Bulk create/update nodes and links (sync <=500 items)" },
      ...
      changeset_async: { method: "POST", path: "/forests/{id}/trees/{subId}/changeset-async", apiType: "org", hint: "id=forestId, subId=treeId, body: changeset config for >500 items" },
